 ...
 * Version $Id: ww.js 4152 2014-07-23 21:28:50Z mcavaliere $
...
/*global Modernizr:true, mlPushMenu, ibmStats */ 
   ...
   */
    ...
    IBM.Common.Widget.MobileMenu = (function() {
        ...
        function insertMobileMenuHtml() {
            ...
            var html = '<div id="m-menu" class="m-menu">';
                    html += '<div id="m-search-module">'; 
                        html += '<form id="m-search-form" action="http://www.ibm.com/Search/" method="get">';
                            html += '<input id="m-q" value="" maxlength="100" name="q" type="text" placeholder="search ibm.com" />';
                            html += '<input type="submit" id="m-search" class="ibm-btn-search" value="Submit"/>';
                        html += '</form>';
                    html += '</div>';

                    html += '<div id="m-menu-scroll">';
                    html += '<div id="m-main-menu">';
                    html += '<h2>IBM.com</h2>';
                    html += '<ul>' + mastLinks + '</ul>';
                    html += '</div>';
                    html += '</div>';
                html += '</div>';

            jQuery('#m-shift').prepend(html);
...
(function($) {
	$(function() {
		if ($('#ibm-primary-tabs')[0] && $('#ibm-secondary-tabs')[0]) {
			ibmweb.queue.push(function() {
				return !!($('#ibm-secondary-tabs')[0] && $('#ibm-secondary-tab-list')[0]);
			}, function() {
				$('<div class="ibm-col-6-5"></div>')
					.append(
						$('#ibm-secondary-tab-list > *')
					)
					.appendTo(
						$('#ibm-secondary-tab-list')
					)
					.after('<div class="ibm-col-6-1"></div>');
			});
			
			ibmweb.queue.push(function() {
				return !!$('#ibm-secondary-tabs > div.ibm-dynamic-subtabs')[0];
			}, function() {
				var $toggleBtn = $('#ibm-expand-container p.ibm-expand-text'),
					toggleBtnViewAllText = $toggleBtn.html();
				
				viewLess();
				
				$toggleBtn.click(function() {
					if ($toggleBtn.html() === toggleBtnViewAllText) {
						viewLess();
					} else {
						viewAll();
					}
				...
				function viewAll() {
					$toggleBtn.appendTo(
						$('#ibm-secondary-tab-list > .ibm-col-6-1')
					);
					
					$('#ibm-secondary-tabs > .ibm-dynamic-subtabs > ul.ibm-tabs > li:last-child').remove();
				}
				
				function viewLess() {
					$('#ibm-secondary-tabs > .ibm-dynamic-subtabs > ul.ibm-tabs').append(
						$('<li></li>').append(
							$toggleBtn
						)
					);
